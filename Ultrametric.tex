...
\begin{definition}
A finite $p$-ultrametric automaton is a sextuple $\langle \Sigma, S, s_0, \delta, A, \lambda \rangle$, where
\begin{itemize}
  \item $\Sigma$ is a finite set -- the input alphabet,
  \item $S$ is a finite set -- the set of states of the automaton,
  \item $s_0:S \rightarrow Q_p$ is the initial state,
  \item $\delta: \Sigma \times S \times S \rightarrow Q_p$ is the transition function; after reading letter $\sigma$ the automaton transitions from state $s(x)$ to state $s_\sigma(y):=\sum_{x\in S}{s(x)\cdot \delta(\sigma, x, y)}$,
  \item $A \subseteq S$ is the set of accepting states,
  \item $\lambda$ is the threshold of acceptance.
\end{itemize}
\end{definition}

\begin{definition}
We say that a finite $p$-ultrametric automaton {\bf recognizes} language $L$ if for any word $w$ in the input alphabet $w \in L$ if and only if after reading the word $\sum_{x \in A}{\left|s_w(x)\right|_p} > \lambda$.
\end{definition}
...
\begin{definition}
A finite ultrametric automaton is called {\bf regulated} if there exist constants $c > 0$ and $\lambda $ such that for arbitrary input word $x$ the norm 
