...
Every rational number is a $p$-adic integer for some prime number $p$. The nature of irrational numbers is more complicated. For instance, $\sqrt{2}$ just does not exist as a $p$-adic number. On the other hand, there is a of continuum of $p$-adic numbers that are not real numbers. Also, for different primes $p_1$, $p_2$ there is a continuum of $p_1$-adic numbers not being $p_2$-adic numbers, and vice versa.
...
Let $w \in \left\{ 0, 1, \ldots, k-1 \right\} ^ m$.
...
If the sum of the norms of the amplitudes in the accepting states is at least $m$ then the word is accepted, otherwise rejected.
...
\begin{figure}[H]
  ...
  Bold arrows show starting states.
...
\begin{figure}[H]
  ...
  Double circled states are accepting.
  Bold arrow shows the starting state.
  Solid lines are $a$-transitions, dashed lines are $b$-transitions.
  All transitions have amplitude 1. }
  \label{autom2}
\end{figure}
